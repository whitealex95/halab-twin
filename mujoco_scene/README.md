...
The web exporter retains each door/drawer's native joint axis, anchor, limits, and affected geometry. The “Open doors & drawers” checkbox opens or closes all articulated doors and drawers; individual asset controls remain available, with a mixed state shown when only some parts are open. Sidebar switches control layers and articulations. Selecting an asset in the list or scene smoothly frames it with room around it in Orbit view; clicking the floor, walls, or empty space clears the selection without resetting the camera; Orbit resets the room view, and manual camera interaction interrupts the movement. Reduced-motion preferences are respected. Door and drawer toggles ease through a short animation (650 ms for hinges, 550 ms for slides), and reversing a toggle continues from its current position. Browser toggles transform those rigid parts, and validation compares the transforms with MuJoCo forward kinematics. The recorded/rendered RGB-D pairs remain at the closed reference configuration. Rounded loose bedding and the bag use simple invisible contact surfaces to remain stable in physics.
